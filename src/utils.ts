...
export function protocolBytesToString(
    bytes: Uint8Array,
    protocol: Protocol | VerificationProtocol,
): string {
    switch (protocol) {
...
export function hubProtocolToVerificationProtocol(
    protocol: Protocol,
...
export function convertProtobufMessageBodyToJson(
    ...
    switch (message.data?.type) {
        ...
        case MessageType.LINK_REMOVE: {
            ...
            body = { type, targetFid }
            if (displayTimestamp) {
                const displayTimestampUnixResult =
                    fromFarcasterTime(displayTimestamp)
        ...
        case MessageType.LINK_COMPACT_STATE: {
            if (!message.data.linkCompactStateBody)
                throw new Error('Missing linkCompactStateBody')
