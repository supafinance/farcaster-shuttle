...
export class EventStreamConnection {
    ...
    async createGroup({
        ...
        try {
            ...
            if (
                groups.some(
                    ([_fieldName, groupName]) => groupName === consumerGroup,
                )
            )
                return
    ...
    private _extractEntries(result: any): { id: string; data: Buffer }[] {
        if (result?.length) {
            return result.map(
                ([idBytes, [, data]]: [Buffer, [Buffer, Buffer]]) => ({
                    id: idBytes.toString(),
                    data,
                }),
            )
...
export class HubEventStreamConsumer {
    ...
    public async processStale(
        ...
        statsd.timing(
            'hub.event.stream.claim_stale_time',
            Date.now() - startTime,
            {
                hub: this.hub.host,
                ...
                await Promise.allSettled(
                    batchedEvents.map((event) =>
                        (async (streamEvent) => {
                            try {
                                statsd.increment(
                                    'hub.event.stream.stale.attempts',
                                    1,
                                    {
                                        hub: this.hub.host,
                                        source: this.shardKey,
                                ...
                                statsd.timing(
                                    ...
                                    Date.now() -
                                        extractEventTimestamp(hubEvent.id),
                                    {
                                        hub: this.hub.host,
                                        source: this.shardKey,
                                    },
                                ...
                                this.log.error(
                                    e,
                                    'Error processing stale event',
                                ) // Report and move on to next event
                            }
                        })(event),
                    ),
                )
